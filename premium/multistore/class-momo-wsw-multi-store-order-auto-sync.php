 ...
 * @since v1.0.0
 */
class MoMo_WSW_Multi_Store_Order_Auto_Sync {
	/**
	 * Constructor
	 */
	public function __construct() {
		add_action( 'admin_init', array( $this, 'momowsw_register_order_auto_sync_settings' ) );

		if ( is_admin() ) {
			add_action( 'add_option_momo_wsw_multistore_order_sync', array( $this, 'momo_wsw_multistore_order_sync' ), 10, 3 );
			add_action( 'update_option_momo_wsw_multistore_order_sync', array( $this, 'momo_wsw_multistore_order_sync' ), 10, 3 );
		}
	}
	 ...
	 */
	public function momowsw_register_order_auto_sync_settings() {
		register_setting( 'momowsw-settings-multistore-order-sync-group', 'momo_wsw_multistore_order_sync' );
	}
	/**
	 ...
	 */
	public function momo_wsw_multistore_order_sync( $old_value, $value, $option ) {
		global $momowsw;
		...
		if ( 'on' === $enable_as_import ) {
			...
			// Enable cron job.
			add_filter( 'cron_schedules', array( $momowsw->premium->fpcron, 'momo_wsw_custom_import_schedule' ), 5 ); // phpcs:ignore WordPress.WP.CronInterval
			$momowsw->premium->fpcron->momo_wsw_enable_order_import_cron();
		} else {
			// Disable cron job.
			$momowsw->premium->fpcron->momo_wsw_disable_order_import_cron();
		}
	}
	/**
	...
	public function momo_wsw_after_as_export_settings( $old_value, $value, $option ) {
		...
		if ( 'on' === $enable_as_export ) {
			...
			$momowsw->premium->fpcron->momo_wsw_enable_product_export_cron();
		} else {
			// Disable cron job.
			$momowsw->premium->fpcron->momo_wsw_disable_product_export_cron();
		}
	}
	/**
	 * Import Scheduled Products from shopify
	 */
	public function momo_wsw_import_shceduled_orders_from_shopify() {
		global $momowsw;
		$crons = wp_get_scheduled_event( $momowsw->premium->fpcron->cron_hooks['iorder'] );
		if ( ! empty( $crons ) ) {
	...
	public function momo_wsw_import_orders_from_date( $from_timestamp ) {
		...
		$multi_servers          = $momowsw->premium->multi->multistore_get_all_server_lists();
		foreach ( $multi_servers as $server ) {
			/**
			 * $momowsw->fn->momo_wsw_run_rest_api( $method, 'fulfillments.json', '', wp_json_encode( $fulfillment ), $shop_url );
			 */
			$shop_url = $server['shop_url'];
			$details  = $momowsw->fn->momo_wsw_run_rest_api( 'GET', 'orders.json', '', $args, $shop_url );
			if ( isset( $details->orders ) && ! empty( $details->orders ) ) {
				...
				$logs   = array(
					'import' => array(
						...
						'msg'  => sprintf( esc_html__( '%1$s order(s) found (%2$s) from date: ', 'momowsw' ), count( $orders ), esc_html( $shop_url ) ) . $pam,
					),
				);
				$momowsw->logs->momo_wsw_save_logs( 'msorder', $logs );
				$count = 0;
				foreach ( $orders as $order ) {
					$response = $momowsw->premium->fn->momo_momowsw_create_order( $order, $shop_url );
					if ( $response ) {
						$count++;
					}
				}
				$logs = array(
					'import' => array(
						...
						'type' => 'success',
						/* translators: %s: prooduct count */
						'msg'  => sprintf( esc_html__( '%1$s order(s) imported successfully from %2$s', 'momowsw' ), $count, esc_html( $shop_url ) ),
					),
				);
				$momowsw->logs->momo_wsw_save_logs( 'msorder', $logs );
			} else {
				$logs = array(
					'import' => array(
						...
						'type' => 'error',
						/* translators: %s: prooduct count */
						'msg'  => sprintf( esc_html__( 'Order not found (%1$s) from date: ', 'momowsw' ), esc_html( $shop_url ) ) . $pam,
					),
				);
				$momowsw->logs->momo_wsw_save_logs( 'msorder', $logs );
			}
